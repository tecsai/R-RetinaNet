...
def mask_valid_boxes(boxes, return_mask=False):
    ...
    mask = (w > 2) & (h > 2) & (ar < 30)  # w/h, h/w最大不超过30
    if return_mask:
        return mask
    else:
        return boxes[mask]
...
def rbox_overlaps(boxes, query_boxes, indicator=None, thresh=1e-1):
    ...
    a_tt = boxes[:, 4]
    a_ws = boxes[:, 2] - boxes[:, 0]  # box width
    a_hs = boxes[:, 3] - boxes[:, 1]  # box height
    a_xx = boxes[:, 0] + a_ws * 0.5 # bbox central x
    a_yy = boxes[:, 1] + a_hs * 0.5 # bbox central y
    ...
    for k in range(K):  # 依次从query_boxes中取边框，与所有的bboxes进行IOU计算
        box_area = b_ws[k] * b_hs[k]
        for n in range(N):
            if indicator is not None and indicator[n, k] < thresh:  # 最小外接正方形的IOU
                continue
            ua = a_ws[n] * a_hs[n] + box_area  # 并集面积
            rtn, contours = cv2.rotatedRectangleIntersection(  # 计算相交多边形
                                                            ((a_xx[n], a_yy[n]), (a_ws[n], a_hs[n]), a_tt[n]),
                                                            ((b_xx[k], b_yy[k]), (b_ws[k], b_hs[k]), b_tt[k]))
            """ 此处需特别注意, rotatedRectangleIntersection返回顶点列表并不一定是按照顺时针或逆时针排列的. """
            """ 解决方法可以通过固定一个点, 然后traverse所有其他点, 计算对应的反正切(arctan), 之后按照arctan从大到小将点进行排列即可. """
            if rtn == 1:
                ia = cv2.contourArea(contours)  # 交集面积
                overlaps[n, k] = ia / (ua - ia)  # 交并比
            elif rtn == 2:
...
def quad_2_rbox(quads, mode='xyxya'):
    ...
    ''' 预先创建 '''
    rboxes = np.zeros((quads.shape[0], 5), dtype=np.float32)  # quads.shape[0]: 标签数量
    for i, quad in enumerate(quads):
        rbox = cv2.minAreaRect(quad.reshape([4, 2]))  # 最小面积旋转矩形   
        x, y, w, h, t = rbox[0][0], rbox[0][1], rbox[1][0], rbox[1][1], rbox[2]  # 中心点(x, y), 宽高(w, h), 旋转角度(t)
        if np.abs(t) < 45.0:
            rboxes[i, :] = np.array([x, y, w, h, t])
        elif np.abs(t) > 45.0:
            rboxes[i, :] = np.array([x, y, h, w, 90.0 + t])
        else:  # 对于等于45度的旋转框, 确保 w > h
            if w > h:
