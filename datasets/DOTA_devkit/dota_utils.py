...
# [SAI-KEY]
class_names = ['II01', 'others']

def custombasename(fullname):
    """
    获取不带路径和后缀的文件名
    """
    return os.path.basename(os.path.splitext(fullname)[0])  # os.path.splitext(fullname): 分离文件后缀，返回元祖，如返回("/2T/000000", ".jpg")
                                                            # os.path.basename: 获取最后一个"/"之后的文件名，如输入"/2T/000000", 输出""000000"

def RecursizeListDir(dir, ext = None):
    """ 
    递归查找文件
    """
    suited_files = []
    needExtFilter = (ext != None)
    for root, dirs, files in os.walk(dir):  # 遍历路径，查找文件
        for filespath in files:
            filepath = os.path.join(root, filespath)
            extension = os.path.splitext(filepath)[1][1:]
            if needExtFilter and extension in ext:
                suited_files.append(filepath)
...
def parse_dota_poly2(filename):
    """
        ...
        [x1, y1, x2, y2, x3, y3, x4, y4]
    """
    objects = parse_dota_poly(filename)  # 从filename中读取识别结果，返回一个列表，列表中每一个元素是dict，包含内容为'conf', 'filename', 'classname', 'poly', 'area'
    for obj in objects:
        obj['poly'] = TuplePoly2Poly(obj['poly'])  # 由[(x, y), (x, y), (x, y), (x, y)]转换为[x, y, x, y, x, y, x, y]
        obj['poly'] = list(map(int, obj['poly']))  # 左右xy转换为int
    return objects
...
def groundtruth2Task1(srcpath, dstpath):
    filelist = RecursizeListDir(srcpath)  # 返回文件列表
    ...
    for filepath in filelist:
        ...
        subname = custombasename(filepath)  # 获取文件名
        pattern2 = re.compile(r'__([\d+\.]+)__\d+___')  # [\d+\.]: 匹配1到多个数字和点(.)的组合成的字符串
...
def Task2groundtruth_poly(srcpath, dstpath):
    thresh = 0.1
    filedict = {}
    Tasklist = RecursizeListDir(srcpath, '.txt')
...
def detections2Task1(srcpath, dstpath):
    filelist = RecursizeListDir(srcpath)
    # names = [custombasename(x.strip())for x in filelist]
    filedict = {}
    for cls in class_names:
        fd = open(os.path.join(dstpath, 'Task1_') + cls + r'.txt', 'w')  # 生成以类别命名的文件, 如生成[integrated_outputs]/Task1_plane.txt
        filedict[cls] = fd
    for filepath in filelist:
        objects = parse_dota_poly2(filepath)  # 从filename中读取识别结果，返回一个列表，列表中每一个元素是dict，包含内容为'conf', 'filename', 'classname', 'poly', 'area'
        ...
        for obj in objects:
            ...
            poly = obj['poly']
            outline = obj['filename'] + ' ' + str(obj['conf']) + ' ' + ' '.join(map(str, poly))  # 存储形式为 [文件名 conf x1 y1 x2 y2 x3 y3 x4 y4]
            filedict[category].write(outline + '\n')                                             # 写到对应类名的文件中，如写到[integrated_outputs]/Task1_plane.txt中

def detections2Task1NoMerge(srcpath, dstpath):
    filelist = RecursizeListDir(srcpath)
    # names = [custombasename(x.strip())for x in filelist]
    filedict = {}
    for cls in class_names:
