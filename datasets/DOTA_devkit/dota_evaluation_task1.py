...
def eval_map(detpath, annopath, imagesetfile, use_07_metric=False, hyps=None):

    # classnames = ['plane', 'baseball-diamond', 'bridge', 'ground-track-field', 'small-vehicle', 'large-vehicle', 'ship', 'tennis-court',
    #             'basketball-court', 'storage-tank',  'soccer-ball-field', 'roundabout', 'harbor', 'swimming-pool', 'helicopter']
    classnames = hyps["names"]
...
'''
[SAI-KEY]
detpath是merge后大图上的15个txt结果的文件夹(按类型划分), 但是注意最后用Task1_{:s}.txt, 或者{:s}.txt输入全部txt文件, 否则报文件夹的错
...
def task1_eval(detpath, val_folder, use_07_metric=False, hyps=None):
