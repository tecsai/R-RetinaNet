...
class RetinaNet(nn.Module):
    ...
    def forward(self, ims, gt_boxes=None, test_conf=None, process=None):
        ...
        features = self.fpn(self.ims_2_features(ims))  # [SAI-KEY] 前向计算，
        cls_score = torch.cat([self.cls_head(feature) for feature in features], dim=1)  # N, (H_0*W_0*NUM_ANCHORS_0 + H_0*W_0*NUM_ANCHORS_0 + H_0*W_0*NUM_ANCHORS_0), NUM_CLASSES
                                                                                        # N, TOTAL_CELLS, NUM_CLASSES
        bbox_pred = torch.cat([self.reg_head(feature) for feature in features], dim=1)  # N, TOTAL_CELLS, 5, 5 for xywha
        
        # bboxes = self.box_coder.decode(anchors_list[-1], bbox_pred, mode='xywht').detach()

        if self.training:
            losses = dict()
            losses['loss_cls'], losses['loss_reg'] = self.loss(cls_score, bbox_pred, anchors_list[-1], gt_boxes, iou_thres=0.5)
            return losses
